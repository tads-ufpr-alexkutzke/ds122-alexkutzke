# Entrega dos exercícios, DS122 (TADSN2)

- **Turma**: TADSN2
- **Semestre**: 2026-02
- **Última atualização**: 14/09/2026 14:59

| GRR | prepare<br>03/09 | HTML<br>03/09 | CSS<br>03/09 | trabalho1<br>13/09 | Laboratório 1<br>17/09 |
| --- | :---: | :---: | :---: | :---: | :---: |
| GRR20214918 | ok | ok (mexeu +10d) | fora do prazo (+10d) | sem fork | sem fork |
| GRR20243354 | ok | ok | sem fork | sem fork | sem fork |
| GRR20250564 | ok | ok | ok | ok | sem fork |
| GRR20252028 | sem commit | sem fork | sem fork | ok | sem fork |
| GRR20260001 | ok | ok (mexeu +1d) | sem fork | sem fork | sem fork |
| GRR20260058 | sem fork | sem commit | sem fork | sem fork | sem fork |
| GRR20260340 | ok | ok | ok | ok | sem fork |
| GRR20260553 | ok | ok | ok | ok | sem commit |
| GRR20260844 | ok | ok | ok | ok | sem commit |
| GRR20260905 | ok | ok | ok | ok | sem fork |
| GRR20261137 | ok | ok | ok (mexeu +5d) | ok | sem fork |
| GRR20261142 | ok | ok | ok | ok | sem fork |
| GRR20261406 | sem grupo | ok (dupla) | ok (mexeu +1d) (dupla) | sem grupo | sem grupo |
| GRR20261426 | ok | ok | sem commit | sem fork | sem fork |
| GRR20261526 | ok | ok | ok | ok | sem fork |
| GRR20261528 | ok | sem fork | sem fork | sem fork | sem fork |
| GRR20261725 | sem fork | sem commit | sem fork | sem fork | sem fork |
| GRR20261773 | sem commit | ok | sem fork | sem fork | sem fork |
| GRR20262101 | ok | ok | ok | sem commit | sem fork |
| GRR20262364 | ok (dupla) | sem grupo | sem grupo | sem grupo | sem grupo |
| GRR20262611 | ok | ok | ok (mexeu +1d) | ok | sem commit |
| GRR20262658 | ok | ok | ok | ok (mexeu +1d) | sem fork |
| GRR20262792 | ok | ok | ok | ok | sem fork |
| GRR20262853 | ok | ok (dupla) | ok (dupla) | ok (dupla) | ok |
| GRR20263207 | ok | ok | ok | ok | ok |
| GRR20263216 | ok | sem commit | fora do prazo (+3d) | ok | sem fork |
| GRR20263366 | ok | ok | ok | ok | ok |
| GRR20263474 | ok | ok | ok | sem fork | ok |
| GRR20263531 | ok | ok | ok | ok | ok |
| GRR20263633 | ok | ok | ok (mexeu +1d) | ok (mexeu +1d) | sem commit |
| GRR20263709 | ok | ok | ok | sem fork | sem fork |
| GRR20264165 | ok | ok | ok | sem fork | sem fork |
| GRR20265011 | sem fork | ok | ok (mexeu +1d) | ok | sem fork |

Legenda: `ok` entregue no prazo; `fora do prazo` só há commits depois da data;
`sem commit` fork criado sem trabalho do aluno (`fork vazio` nunca teve um
commit, `ramo errado` tem commits fora do ramo padrão); `sem fork` a tarefa não
foi bifurcada; `sem acesso` o grupo existe, mas o professor não foi adicionado
como reporter; `sem grupo` o grupo não foi criado, ou foi criado privado sem
compartilhar. O sufixo `(dupla)` marca entrega achada no fork do colega.

Se a sua linha mostra `sem grupo` ou `sem acesso`, confira em **Settings >
General** se o grupo se chama `ds122-2026-2-n-grrxxxxxxxx`, se está compartilhado (não privado sem
convite) e se `alexkutzke` consta em **Members** como `reporter`.
